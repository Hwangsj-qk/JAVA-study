...
public class Stream8 {
    ...
    public static void main(String[] args) {
        ...
        // 리턴값을 갖는다 = 최종연산 = 다른 최종연산(ex. forEach)를 쓸 수 없음

        Stream<Integer> numberStream = numberList.stream();

        // 에러 발생 : IllegalStateException(stream has already been operated upon or closed)
        // 최종 연산을 한 번이라도 하게 되면 다른 최종 연산을 할 수 없다.
        //boolean b1 = numberStream.allMatch(number -> number > 0);
        //boolean b2 = numberStream.anyMatch(number -> number > 0);

        // anyMatch : 조건 만족이 하나라도 되는지 확인
        boolean hasEven = numberList.stream()
                .anyMatch(number -> number % 2 == 0);// 짝수 존재 여부(boolean)
        System.out.println("짝수가 하나라도 있나요? " + hasEven);

        // noneMatch
        boolean greaterThan5 = numberList.stream()
                .noneMatch(number -> number > 5);// 5를 초과하는 것이 없는지
        System.out.println("5를 초과하는 수가 없습니까? " + greaterThan5);

        // 문자열 리스트의 유효성 검증시
        List<String> words = Arrays.asList("사과", "바나나", "배");
        boolean allNoEmpty = words.stream().anyMatch(word -> !word.isEmpty());
        ...
        boolean isAllExpensive = products.stream().allMatch(product -> product.price >= 300);
