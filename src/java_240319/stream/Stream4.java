...
public class Stream4 {
    ...
    public static void main(String[] args) {
        ...
        - 지연 평가: 최종 연산 전에는 실해되지 않음
        filter : 조건에 맞는 요소만 남김
        map : 매개변수의 함수에 맞게 요소를 반환
        stored : 요소를 정렬 (Comparator)
        distinct : 중복되지 않는 고유값만 남김
        limit : 스트림 처음부터 n까지만 남김
        skip : 스트림 처음부터 n까지 버림
        peek : 각 요소를 순회하면서 Consumer 작업을 실행지연 (Stream 에 영향을 끼지치 않음 -> 지연 평가)
        ...

        최종연산
        - 실제로 계산을 실행하고, 리턴값을 갖거나, side Effect 를 발생시킴
        - 최종 연산 시 스트림을 모두 소모해서, 더 이상 스트림 사용 불가
        forEach : 각 요소를 순회하면서 Consumer 작업을 실행
        ...
        min : Comparator 에 맞는 최솟값 반환
        max : Comparator 에 맞는 최댓값 반환
        ...
         */
