...
public class Ex8 {
    ...
    public static void main(String[] args) {
        ...
        boolean allMatch = userBookRentals.entrySet().stream()
                .allMatch(user -> user.getValue().size() >= 1);

        if (! allMatch) {
            System.out.println("책을 대출하지 않은 사용자가 있습니다. ");
        } else {
            System.out.println("모든 사용자가 책을 대출하였습니다. ");
        }
        ...
        boolean theGreatGatsby = userBookRentals.values().stream()
                .noneMatch(book -> book.contains("The Great Gatsby"));
        if(theGreatGatsby) {
            System.out.println("아무도 The Great Gatsby 책을 대출하지 않았습니다.");
        } else {
            System.out.println("The Great Gatsby 책은 대출 이력이 있습니다.");
        }
