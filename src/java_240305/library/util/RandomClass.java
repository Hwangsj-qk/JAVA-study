...
public class RandomClass {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            // bound를 6까지 설정하면 : 0~5에서 랜덤하게 나오므로 주사위를 설정하기 위해서 +1 이 필요하다.
        ...
        // 랜덤한 0.0 ~ 1.0까지 '실수' 범위의 값을 반환
        double d1 = random.nextDouble();
        double d2 = random.nextDouble();
        double d3 = random.nextDouble();

        // 해당 범위 내에서 실수 난수 생성(첫 인자 포함, 마지막 인자 제외)
        double d4 = random.nextDouble(3.14, 6.28);
        int i4 = random.nextInt(1, 7);
        // Math와 다른점: 범위가 지정할 수 있음

        // 랜덤한 참 거짓 생성
        boolean trueFalse = random.nextBoolean();
        System.out.println(trueFalse);

        // 시드 값 : 난수 생성의 시작점을 제공하고, 동일한 시드값을 생성된 Random 객체는 동일한 난수 생성
