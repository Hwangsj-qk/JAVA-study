...
public class HashCode {
    public static void main(String[] args) {
        ...
        // equals() 메서드를 오버라이드해서 동등성(동일성 X)을 비교하는 경우,
        // hashCode() 메서드도 함게 오버라이드하여, 동등한 객체가 나오도록 반환해야 한다. => 일관성이 중요함.
        // (hash 코드 바뀌면 equals 도 바꿔야함)
        // HashSet, HashMap, HashTable 등 hash 값을 이용하는 자료구조에서
        // 동등성을 비교할 때
        // 먼저 hashCod() 값을 비교함 => 다르면 다른 객체, 같으면
        // 다음으로 equals() 값을 비교함 => false 면 다른 객체, true 면 동등 객체
        // hashCode 는 hash 충돌이 발생할 수 있다 : 서로 다른 입력 값에 대해서 동일 한 해시값이 생성되는 경우,
        // 잘 만든 해시함수는 hash 충돌을 최소화하지만 완전히 hash 충돌이 없을 수는 없다.
