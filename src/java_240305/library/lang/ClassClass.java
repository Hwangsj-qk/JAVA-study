...
import java_240221.static0.Obj;
...
public class ClassClass {
    public static void main(String[] args) throws ClassNotFoundException, InstantiationException, IllegalAccessException, NoSuchMethodException {
        ...
        // Class 클래스는 클래스에 대한 메타 데이터, 다양한 데이터를 조회, 조작, 관리함
        ...
        // 일반적이지 않은 클래스 인스턴스 생성하기
        String newInstance = stringClass.newInstance();

        // 클래스에 선언된 모든 메서드를 배열로 리턴
        Method [] declareMethods = stringClass.getDeclaredMethods();

        // 해당 클래스의 모든 메서드 시그니처 확인
        for (Method declarMethod : declareMethods) {
            System.out.println(declarMethod);
        }
...
//        arrList.add("a");     호출 불가
        ...
        /*
        Reflection 주의사항
        프레임워크를 새로 개발하거나, 컴파일 시점이 아닌 런타임에서 클래스와 상호작용할 때 매우 유용히 사용할 수 있음.
