...
public class DaemonThread {
    ...
    public static void main(String[] args) {
        ...
        Runnable rhythm = () -> {
            ...
            while (true) {
                System.out.println(rhyme.charAt(index++) + " ");
                // index와 rhyme.length를 나눈 나머지 값을 index에 할당
                index %= rhyme.length();
        ...
        Thread singThread = new Thread(() -> {
            ...
            // 리듬 스레드를 노래 스레드의 데몬 스레드(보조 스레드)로 설정
            // 노래 스레드(주 스레드)가 종료되었을 때 리드 스레드(데몬 스레드)도 함께 종료
