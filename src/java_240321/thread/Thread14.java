...
public class Thread14 {
    ...
    public static void main(String[] args) {
        ...
        List<String> cusmtomerList = Arrays.asList("둘리", "철수", "영희", "희동", "길동");

        cusmtomerList.stream()
                .forEach(customer -> {
    ...
    class CoffeeMachine {
        ...
        synchronized public void takeOut(Customer customer) {
            while (cups <= 0) {
                System.out.printf("[컵: %d] %s : 커피가 없습니다. \n", cups, customer.name);
                try {
                    wait(); // 버퍼가 비어있으면 대기
            ...
            // 버퍼를 하나 줄임
            System.out.printf("[컵: %d] %s : 커피 테이크아웃 해가요 \n", cups, customer.name);
            cups--;
            notifyAll();
        ...
        synchronized public void fill() {
            // 커피 컵이 여유가 있으면 대기
            while (cups > 3){
                System.out.printf("[컵: %d] : 매니저 : 재고 여유 있습니다.\n", cups);
                try {
                    wait();
                } catch (InterruptedException e) {}
            }
