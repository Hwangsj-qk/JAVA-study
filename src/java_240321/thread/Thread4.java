...
public class Thread4 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(mainThread.getName() + " 스레드");       // main -> 스레드 이름 가져오기
      ...
      thread0.start();
        ...
        Thread threadChat = new Thread() {
            @Override
            public void run() {
                // 인스턴스 내부에서 이름을 반환하여 출력 (인스턴스 바깥에서도 가능)
                System.out.println(getName() + " 스레드");
            }
        };
        ...
        // 작업 스레드의 이름을 변경하여 설정
        threadChat.setName("Chatting");     // 스레드 이름 설정
        threadChat.start();
