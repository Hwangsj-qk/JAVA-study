...
public class Ex7_Hangman {
    public static void main(String[] args) {
        ...
        // 플레이어에게 보여줄 단어의 형태 ('_'로 가려진 형태) -> 선택된 단어의 길이 (ex. java -> 4개) 만큼의 캐릭터 타입의 배열 생성
        ...
        for (int i = 0; i < displayArray.length; i++) {
            ...
            boolean wordComplted = false;

            // 최대시도 횟수를 넘어서거나, 단어가 완성되면 종료
            while (tries < MAX_TRIES && !wordComplted) {
                System.out.print("단어 추측: ");
                printDisplayArray(displayArray);
                // 현재까지 맞춘 단어의 상태 출력하는 메서드 -> "_ _ _ _"로 표시
                ...
                wordComplted = isWordCompleted(displayArray);
            }
