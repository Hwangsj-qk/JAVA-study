...
public class Function3 {
    public static void main(String[] args) {
       ...
       // 버튼 객체 생성 및 클릭 리스너 설정
        ...
        // 버튼을 클릭하는 함수(람다식을) 함수형 인터페이스에 저장
        ...
        // character을 int 형을 캐스팅하면서 유니코드 불러오기

        boolean b1 = isSameNumber.test('A', 64);
        boolean b2 = isSameNumber.test('A', 65);

        System.out.println("A는 유니코드 64인가? " + b1);
        System.out.println("A는 유니코드 65인가? " + b2);

        // BiFunction : 입력을 2가지로 받아 결과를 리턴, 타입이 다를 수 있음
        BiFunction<String, String, Integer> concatLength =
                (s1, s2) -> (s1 + s2).length();
        Integer length = concatLength.apply("문자열 합쳤을 때", "길이를 정수로 반환");
        System.out.println("length = " + length);

        // UnaryOperator(단항) : 입력 타입과 결과 타입이 모두 같을 때
        UnaryOperator<Integer> powerOperator = x -> x * x;
        System.out.println(powerOperator.apply(5));

        // UnaryOperator(이항) : 입력 타입과 결과 타입이 모두 같을 때
        BinaryOperator<Integer> sumOperator = (x,y) -> x + y;       // 매개변수 2개 받기
