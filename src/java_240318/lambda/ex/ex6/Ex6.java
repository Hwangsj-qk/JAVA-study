...
public class Ex6 {
    ...
    요구사항
    주어진 코드를 바탕으로, 학생들의 영어와 수학 평균 점수를
    계산하는 프로그램을 작성하세요. 프로그램은 다음 요구사항을 충족해야 합니다.

    Student 클래스 정의:
    Student 클래스에는 학생의 이름(String), 영어 점수(int), 수학 점수(int)를 필드로
     가집니다.
    Student 클래스는 생성자를 통해 이러한 정보를 초기화할 수 있어야 합니다.
    getEnglishScore() 메서드와 getMathScore() 메서드를 통해
    각각의 점수를 반환할 수 있어야 합니다.

    평균 점수 계산:
    avg(Function<Student> function) 메서드는
    Function<Student, Integer> 함수형 인터페이스를 매개변수로 받아,
    학생들의 평균 점수를 계산합니다.
    avg 메서드는 모든 학생들에 대해 주어진 함수형 인터페이스를 적용한 후,
    그 결과값들의 평균을 반환합니다.
