...
public class Exception1 {
    public static void main(String[] args) {
        ...
        // 실행 중에는 발생하는 오류, 컴파일 단계에서 오류가 발생하지 않음
        RuntimeException runtimeException;      // 실행 시 발생하는 예외를 총괄
        NullPointerException nullPointerException;      // null에 참조 접근시 발생
        IndexOutOfBoundsException indexOutOfBoundsException;        // 유효하지 않은 인덱스에 접근할 때 발생
        ArithmeticException arithmeticException;    // 수학적 계산이 불가능할 때
        IllegalArgumentException illegalArgumentException;      // 부적합한 인자를 전달했을 때
        ClassCastException classCastException;      // 잘못된 타입 캐스팅을 할 때
