...
password.matches(".*[A-Z].*") // 최소 하나의 대문자 포함
    ...
    public static void main(String[] args) throws Ex5 {
    ...
    public static void test() throws Ex5 {
        if (password.length() < 8) {
            throw new Ex5("비밀번호는 최소 8자 이상이어야 합니다.");
        } else if(! password.matches(".*[0-9].*")) {
            throw new Ex5("비밀번호에는 최소 하나의 숫자가 포함되어야 합니다.");
        } else if (!password.matches(".*[A-Z].*")) {
            throw new Ex5("비밀번호에는 최소 하나의 대문자가 포함되어야 합니다");
        }
